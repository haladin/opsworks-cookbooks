...
if node[:ssl_support] and node[:ssl_key_gen]
	...
	Chef::Log.info(`sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -subj #{node[:ssl_certificate_subj]} -keyout #{node[:ssl_certificate_path]}/#{node[:ssl_certificate_key_name]} -out #{node[:ssl_certificate_path]}/#{node[:ssl_certificate_name]}`)	
end
